...
/**
 * The two facts the old API let callers assert, now derivable only.
 * External packages cannot reach `DERIVED` (the barrel omits it and the
 * package `exports` map blocks deep imports), so this interface has no
 * constructible literal outside core.
 */
...
export function assertedWorld(
    ...
    return { observedMeanings, preconditionHolds, [DERIVED]: true };
}
