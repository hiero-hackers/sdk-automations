...
/**
 * A GitHub that answers by ROUTE rather than in order — the first matching URL
 * fragment wins, and anything unrouted is a 404.
 *
 * One sweep record costs four or five calls in an order the reader owns, so a
 * positional script would pin the reader's call order rather than its reading,
 * and would have to be rewritten every time a read moved. The 404 is what makes
 * a missing route a failing test instead of a silently reused body.
 */
export function routed(routes: Readonly<Record<string, ResponseStep>>): ResponseStep {
    return (url, init) => {
        for (const [fragment, step] of Object.entries(routes)) {
            if (!url.includes(fragment)) continue;
            if (step instanceof Error) throw step;
            return typeof step === "function" ? step(url, init) : step;
        }
        return failure(404, '{"message":"no route"}');
    };
}

/**
 * One route's answer, FRESH on every call: a `Response` body may be read once,
 * so a route hit twice — a linked issue named by two pull requests, a second
 * page under the same matcher — would answer the second caller with nothing.
 */
export const json =
    (value: unknown, headers?: HeadersInit): ResponseStep =>
    () =>
        success(JSON.stringify(value), headers);
