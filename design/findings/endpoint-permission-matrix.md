...
| Redirect | 3xx with `location` — documented for renamed repos (301) and some downloads (302); **never observed in a probe**. The client refuses to follow and classifies as `redirected` | transient 5xx (which is worth a retry; a 301 never is) | documented only — re-probe obligation |
